/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UserUpdateManyWithoutCollegesInput } from "./UserUpdateManyWithoutCollegesInput";
import {
  ValidateNested,
  IsOptional,
  IsString,
  MaxLength,
} from "class-validator";
import { Type } from "class-transformer";
import { DepartmentUpdateManyWithoutCollegesInput } from "./DepartmentUpdateManyWithoutCollegesInput";

@InputType()
class CollegeUpdateInput {
  @ApiProperty({
    required: false,
    type: () => UserUpdateManyWithoutCollegesInput,
  })
  @ValidateNested()
  @Type(() => UserUpdateManyWithoutCollegesInput)
  @IsOptional()
  @Field(() => UserUpdateManyWithoutCollegesInput, {
    nullable: true,
  })
  users?: UserUpdateManyWithoutCollegesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => DepartmentUpdateManyWithoutCollegesInput,
  })
  @ValidateNested()
  @Type(() => DepartmentUpdateManyWithoutCollegesInput)
  @IsOptional()
  @Field(() => DepartmentUpdateManyWithoutCollegesInput, {
    nullable: true,
  })
  departments?: DepartmentUpdateManyWithoutCollegesInput;
}

export { CollegeUpdateInput as CollegeUpdateInput };
